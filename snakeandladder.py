...
def main():
    ...
    n = int(input())
    for i in range(n):
        character = [int(i) for i in input().split()]
        snake_and_ladders[character[0]]=character[1]
    m = int(input())
    moves = [int(i) for i in input().split()]
    for i in range(m):
        if i %2==0:
            scoreA+=moves[i]
            while scoreA in snake_and_ladders:
                scoreA=snake_and_ladders[scoreA]
            if scoreA>=99:
                print("A 99")
                return
        else:
            scoreB+=moves[i]
            while scoreB in snake_and_ladders:
                scoreB=snake_and_ladders[scoreB]
            if scoreB>=99:
                print("B 99")
                return
    if scoreA>scoreB:
